...
public class ScenarioUpdate extends Task {
	...
	public void doTask(HttpServletRequest request, HttpServletResponse response) {
		...
		String toInsertOK = "{a:" + toInsert.replace("[[{", "[{").replace("}]]", "}]").replace("}],[{", "},{") + "}";
		System.out.println(toInsertOK);
		String toUpdate = request.getParameter("toUpdate");
		String toUpdateOK = "{a:" + toUpdate.replace("[[{", "[{").replace("}]]", "}]").replace("}],[{", "},{") + "}";
		String toDelete = request.getParameter("toDelete");
		String toDeleteOK = "{a:" + toDelete.replace("[[{", "[{").replace("}]]", "}]").replace("}],[{", "},{") + "}";
		String zToInsert = request.getParameter("zToInsert");
		String zToInsertOK = "{a:" + zToInsert.replace("[[{", "[{").replace("}]]", "}]").replace("}],[{", "},{") + "}";
		...
		String zToUpdateOK = "{a:" + zToUpdate.replace("[[{", "[{").replace("}]]", "}]").replace("}],[{", "},{") + "}";
		String zToDelete = request.getParameter("zToDelete");
		String zToDeleteOK = "{a:" + zToDelete.replace("[[{", "[{").replace("}]]", "}]").replace("}],[{", "},{") + "}";
			...
			if(!(toInsertOK.equals("{a:[]}")) && !(toInsertOK.equals("{a:}"))){
				...
				try {
					...
					for(int it1 = 0; it1 < array3.length(); it1++){
						...
						zona.add(array3.getJSONObject(it1).getString("nome"));
			...
			if(!(toUpdateOK.equals("{a:[]}")) && !(toUpdateOK.equals("{a:}"))){
				...
				try {
					...
					for(int i = 0 ; i < array4.length() ; i++){
						...
						tc_territorio_id.add(array4.getJSONObject(i).getString("tc_territorio_id"));
						z_id.add(array4.getJSONObject(i).getString("zona_id"));
						System.out.println("z_id di array4: "+z_id.get(i));
						queryUpdate += "zona_id='" + z_id.get(i) + "' ";
						
						queryUpdate += "WHERE EXISTS (SELECT zona.zona_id "
												 + "FROM zona "
												 + "LEFT OUTER JOIN scenario ON zona.scenario_id = scenario.scenario_id "
												 + "WHERE scenario.scenario_id = " + scenario_id
												 + ") ";
						queryUpdate += "AND tc_territorio_id='" + tc_territorio_id.get(i) + "' ";
