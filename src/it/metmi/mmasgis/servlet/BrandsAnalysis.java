 ...
 * 
 */
public class BrandsAnalysis extends Task {
	...
	public void doTask(HttpServletRequest request, HttpServletResponse response) {
		...
		String tabellaTemporanea = String.format(Const.tempTable,"mar","mar", selGeo, ListTask.getValoriFiltri(request));
		...
		if (db.connetti()) {
			...
			if (db.eseguiAggiornamento(tabellaTemporanea)) {
				...
				/*
				 * creo una tabella temporanea dove trovo i PV che utilizzano ogni singola marca a seconda della classe
				 * di marca scelta e dopodichè la mette in join con la query che si occupa dell'aggregazione marche
				 */
				
				String qq1=String.format(Const.tempTableBrandNumPv,censimento,censimento,censimento,classeId);
				if (db.eseguiAggiornamento(qq1)) {					
					String query = String.format(Const.analisiMarche, base_num, base_pot, base_pot, base_num, base_pot, censimento, censimento,censimento, censimento, censimento, classeId, sortName, order);
