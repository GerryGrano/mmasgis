...
/**
 * Classe che svolge l'"Analisi marche Azienda", estraendo i dati dal censimento selezionato
 * e calcolando i vari indici che compongono l'analisi, sulla base della marca passata 
 * come parametro nella richiesta HTTP
 *
 */
public class BrandsAziendaAnalysis extends Task {
	...
	/**
	 * Dati il censimento e la classe della marca aziendale(estratti dalla richiesta HTTP), crea una tabella temporanea di 
	 * appoggio ed esegue su di essa la query per l'analisi marche azienda. 
	 * 
	 */
	@Override
	public void doTask(HttpServletRequest request, HttpServletResponse response) {
		...
		String tabellaTemporanea = String.format(Const.tempTable,"mar_az","mar_az", selGeo, ListTask.getValoriFiltri(request));
		
if (db.connetti()) {
			
			if (db.eseguiAggiornamento(tabellaTemporanea)) {
				
				Vector<String[]> baseField = db.eseguiQuery(String.format(Const.baseNumPotAz, censimento, censimento, censimento, classeId));
				...
				/*
				 * creo una tabella temporanea dove trovo i PV che utilizzano ogni singola marcaAz a seconda della classe
				 * di marcaAz scelta e dopodichè la mette in join con la query che si occupa dell'aggregazione marcheAz
				 */
				String qq1=String.format(Const.tempTableBrandAzNumPv,censimento,censimento,censimento,classeId);
				if (db.eseguiAggiornamento(qq1)) {					
					String query = String.format(Const.analisiMarcheAz, base_num, base_pot, base_pot, base_num, base_pot, censimento, censimento,censimento, censimento, censimento, classeId, sortName, order);
