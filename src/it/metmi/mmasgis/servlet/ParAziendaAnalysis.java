...
/**
 *
 * Classe che opera l'Aggregazione Parametri Azienda
 *
 */
public class ParAziendaAnalysis extends Task {
	...
	/**
	 * Dati il censimento, la classe di parametri Aziendali, e il livello su cui operare l'analisi,
	 * crea la tabella temporanea contenente i PV della selezione geografica (ed eventuali filtri),
	 * e su tale tabella esegue la query per l'aggregazione parametri aziendali.
	 * <p>
	...
	public void doTask(HttpServletRequest request, HttpServletResponse response) {
		...
		String tabellaTemporanea = String.format(Const.tempTable,"par_az","par_az", selGeo, ListTask.getValoriFiltri(request));
		String query = "";
		boolean b = true;

		if (livello.equals("reg")) {
			query = String.format(Const.aggregazioneParAziendaReg, classeId);
		} else if (livello.equals("pro")) {
			query = String.format(Const.aggregazioneParAziendaPro, classeId);
		} else if (livello.equals("com")) {
			query = String.format(Const.aggregazioneParAziendaCom, classeId);
		} else if (livello.equals("cap")) {
			query = String.format(Const.aggregazioneParAziendaCap, classeId);
			b=false;
		}
		...
		if (db.connetti()) {
			if (db.eseguiAggiornamento(tabellaTemporanea)) {
				...
				Vector<String[]> v = db
						.eseguiQuery("SELECT tc_par_az_id AS id, tc_par_az.testo AS text, tc_par_az.tc_clpar_az_id AS class_id, "
								+ "tc_clpar_az.testo AS class_text FROM tc_par_az JOIN tc_clpar_az ON tc_par_az.tc_clpar_az_id=tc_clpar_az.tc_clpar_az_id "
								+ "WHERE tc_par_az.tc_clpar_az_id=" + classeId + " ORDER BY tc_par_az.ordine ASC");
				...
				for (int i = 0; i < rs.size()+1; i++) {
					...
					if(i==rs.size()) {
						...
						if (!(rs.get(i).get("id").equals(prevId)) && true) {
							// lo inserisco
							tmp.put("liv", rs.get(i).get("nome1").replace(".", ""));
				...
				for (int k = 0; k < rs.size(); k++) {
					for (int w=0; w<result.size(); w++) {
						...
						if (result.get(w).get("liv").equals(rs.get(k).get("nome1")) && result.get(w).get("idn").equals(rs.get(k).get("id"))) {
				...
				for(int n=0; n<result.size(); n++) {
					...
					while (keyIter.hasNext()) {
						...
						if(!(key.equals("liv")) && !(key.equals("tot"))) {
					...
					result.get(n).put("tot", String.valueOf(tot));
