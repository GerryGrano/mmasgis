...
tree = Ext.create('Ext.tree.Panel', {
	...
	listeners : {
		...
		itemclick : function(view, rec, item, index, eventObj) {
			...
			//costruisco il fid per cercare se è gia presente sulla mappa
			var fid = fromNomiToFid(rec.raw.layer,rec.raw.codice);

			//se è non è presente lo aggiungo allo scenario e alla zona selezionata nel box
			if (containsFromFid(select.features, fid) == false) {

				//constriusco record per inserirlo negli store
				var data = {
						colore:ZonaSelezionata[2],
						nome:ZonaSelezionata[1],
						nome_territorio:rec.raw.text,
						sigla:rec.raw.sigla,
						tabella_territorio:rec.raw.layer,
						tc_territorio_id:""+(parseInt(rec.raw.codice) + 1),
						zona_id:ZonaSelezionata[0]
						};
				//aggiungo alla zona e allo scenario
				myDataBox.push(data);
				myDataScenario.push(data);

				//ordino gli store
				//myDataBox.sort(orderByNomeTerritorio);
				myDataScenario.sort(orderByZona);

				//visualizzo in box
				Ext.getCmp('gridSel').getStore().loadData(myDataBox, false);

				//visualizzo sulla mappa la feature
				addFeaturesToMap(fid);
			}
			//se è gia presente..cosa faccio? da implementare..
		}
	},
	dockedItems : [{
		xtype : 'toolbar',
