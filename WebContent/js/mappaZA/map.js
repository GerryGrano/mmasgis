...
//		}
	...
	if (admin_azienda==0){
		...
		setTimeout(function(){alert("I territori che sto caricando ti sono stati assegnati da un tuo responsabile, puoi lavorare esclusivamente su quelli");},2500);
		
	}else
		setTimeout(function(){alert("Hai diritto a lavorare su tutta l'italia, sto caricando tutta cartina.");},2500);
...
function addFeaturesToGrid(feature) {
	...
	if (Ext.getCmp('zone_analysis_panel').isVisible()==false){
		...
		Ext.getCmp('gridSel').getStore().loadData(myDataScenario, false);
	}else{
		myDataBox.push(data);
		myDataScenario.push(data);
...
function removeFeaturesFromGrid(featureFid) {
	...
	//se la ZA non è aperta non esiste nessuna zona, carico lo scenario nel box ed esco
	if (Ext.getCmp('zone_analysis_panel').isVisible()==false){
		Ext.getCmp('gridSel').getStore().loadData(myDataScenario, false);
		return;
	}
...
function showFeatures(database, custom) {
	...
	//se non è aperta la ZA passo i dati di tutto myDataScenario
	if (Ext.getCmp('zone_analysis_panel').isVisible()==false)
		dati=myDataScenario;
	else
		dati=myDataScenario;
	...
	for (feature in dati){
		r++;
		var fid = fromNomiToFid(dati[feature].tabella_territorio, dati[feature].tc_territorio_id - 1);
		
		var type = dati[feature].tabella_territorio;
		

		if (type.substring(0,3).toLowerCase() == "reg") {
			f.reg.value = f.reg.value + selectionControl.features[fid].attributes['COD_REG'] + ",";
		}
		if (type.substring(0,3).toLowerCase() == "pro") {
			f.pro.value = f.pro.value + selectionControl.features[fid].attributes['COD_PRO'] + ",";
		}
		if (type.substring(0,3).toLowerCase() == "com") {
			f.com.value = f.com.value + selectionControl.features[fid].attributes['PRO_COM'] + ",";
		}
		if (type.substring(0,3).toLowerCase() == "cap") {
			f.cap.value = f.cap.value + selectionControl.features[fid].attributes['nome'] + ",";
		}
...
function Disgrega(btn) {
	...
	for (n in myDataScenario) {
		if (myDataScenario[n].tc_territorio_id == cod && myDataScenario[n].tabella_territorio.substring(0, 3) == layer.substring(0, 3)) {
			...
			colore_framm=ZonaFrammentata[2];
			break;
		}
	}
...
function getChildren(fid, requestFid) {
	...
	// se la ZA è chiusa allora vuol dire che non devo fare distinsione sul colore da frammentare
	if (Ext.getCmp('zone_analysis_panel').isVisible()==false)
	...
	Ext.Ajax.request({
		...
		callback: function (opt, success, response) {
			...
			console.log(ZonaSelezionata[2]);

			if (ZonaSelezionata[2]==null)
...
function visualizzaFiltro(request) {
	...
	addToFiltro(feat);
	//aggiungo feature all albero Italia
	addToTree(feat);
